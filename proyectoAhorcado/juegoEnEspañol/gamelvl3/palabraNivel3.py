...
def ahorcadoNvl3():
 ...
 if eleccion == "1":
    ...
    cursor = conn.cursor() 
    instruccion = f"SELECT palabra FROM datosNivel3"
 ...
 conn = sql.connect("datosAhorcado.db")
 cursor = conn.cursor() 
 instruccion = f"SELECT palabra, pista, Descripcion FROM datosNivel3 where palabra = '{name}'"
 cursor.execute(instruccion)
 datos = cursor.fetchall()
 lista = []
 for n in datos:
